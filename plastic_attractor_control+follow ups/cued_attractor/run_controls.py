...
omit_practice_combo) rather than model-version comparison.
...
try:
    ...
    # fall back to the working directory, which should be either the project
    # root or the cued_attractor folder itself.
    _this_dir = os.getcwd()
_project_root = (
    os.path.dirname(_this_dir)
    if os.path.basename(_this_dir) == 'cued_attractor'
    else _this_dir
)
...
from cued_attractor.task import ALL_STIMULI
from model_versions_config import model_versions

output_dir = os.path.join(_project_root, 'cued_attractor', 'output', 'controls')
...
def real_accuracy(result):
    ...
    practice -- matches the old flat script's convention."""
...
def exposure_curve(sequences, n_bins=n_exposure_bins):
    """Mean +/- SEM accuracy at each exposure number (1st, 2nd, ...), pooling
    one value per seed per bin -- each seed contributes at most one accuracy
    per exposure index, so bins stay independent across seeds."""
...
def first_exposure_trial_index(sequence, config):
    ...
    TrialResult.block_index is the block's position in the whole session
    (practice, then performance practice, then real blocks), so it must be
    offset by however many practice/performance-practice blocks came first,
    not multiplied by num_trials directly -- that undercounted nothing but
    double-counted the offset, inflating "trial 0 of the first real block"
    into "trial num_practice_blocks*num_trials" instead of 0.
    """
...
if __name__ == '__main__':
    ...
    control1_results = run_seeds_parallel(
        build_configs(shuffle_cues_test=True), label='control1'
    )
    ...
    # paired by seed: each seed contributes one matched pair, consistent with
    # the per-seed SEM used throughout the rest of this package.
    ...
    # ---------------- EXPOSURE CURVES ----------------
    # accuracy at each novel-cue / omitted-combo exposure number, so a fast
    # first exposure (still re-learning) isn't averaged together with a late
    # one (already adapted) -- see analysis.py's docstring on these two
    # sequence functions for why that distinction matters.
    ...
    first_novel_trial = [
        first_exposure_trial_index(seq, r.config)
        for seq, r in zip(novel_sequences, control2_results) if seq
    ]
    ...
    first_omitted_trial = [
        first_exposure_trial_index(seq, r.config)
        for seq, r in zip(omitted_sequences, control3_results) if seq
    ]
    ...
    summary = {
        ...
        'control2_trained_sem': _sem(trained_acc_c2),
        'control2_novel_mean': float(np.mean(novel_acc_c2)),
        'control2_novel_sem': _sem(novel_acc_c2),
        ...
        'control3_other_sem': _sem(other_acc_c3),
        'control3_omitted_mean': float(np.mean(omitted_acc_c3)),
        'control3_omitted_sem': _sem(omitted_acc_c3),
