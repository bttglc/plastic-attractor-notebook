...
def _balanced_shuffled_deck(
    ...
    *,
    ...
    (cue, stimulus) pairing from the deck (Control 3). Since a never-shown
    cue never externally drives model.step's cue input, its cue-to-gate
    weights never accumulate an eligibility trace either -- restricting the
    deck here keeps the gate from learning the restricted/omitted cue too,
    with no separate gate-specific masking needed.
    """
...
def real_block_plan(
    ...
    *,
    ...
    "novel" cue or combo, and the gate weights tied to it, stay genuinely
    untrained until the real switching blocks below in
    run_switching_experiment). Stimulus identity is drawn first and is
    ...
    (Control 1). Leave allowed_cues_by_task and omit_combo unset when
    shuffle_cues=True -- real (test) blocks are unrestricted in Controls
    2/3, only practice is.
    """
...
class SwitchingExperimentConfig:
    ...
    # instruction and, if enabled, performance practice), e.g.
    # {Task.COLOR: (color_cue_0,), Task.SHAPE: (shape_cue_0,)}, so the other
    # cue(s) -- and the gate weights tied to them -- are novel at the real
    # switching blocks. None = no restriction.
    ...
    # test a combination the network -- and the gate -- never saw in training.
...
def run_switching_experiment(
    ...
    # here so the restricted cue or omitted combo is never taught -- to the
    # main W weights or, since it's never shown as input, to the gate either.
    ...
    # both the main W weights and the gate's own cue-input weights (via its
    # eligibility trace, unforced here). The same restriction/omission
    # carries over, otherwise a "novel" cue or combo would already have been
    # trained on -- gate included -- by the time the real switching blocks
    # run, defeating Controls 2 and 3.
