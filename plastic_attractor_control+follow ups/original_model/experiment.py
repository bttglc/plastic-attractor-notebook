...
class BlockedExperimentConfig:
    ...
    # Control 3, adapted for this blocked, cue-less paradigm: this model has
    # no cue to shuffle (Control 1) or restrict-then-generalize (Control 2),
    # since the "cue" and the rule are the same thing here -- fully re-taught
    # every block via instruction. Omission still translates: instruction
    # only teaches the abstract feature-level mapping (e.g. "green -> left"),
    # never a specific stimulus combo, so omitting one (task, stimulus) combo
    # from every block's trials until omit_combo_until_block, then letting it
    # appear normally, tests whether the network responds correctly to a
    # combo it was never shown, from the abstract rule alone.
    omit_combo: tuple[Task, Stimulus] | None = None
    omit_combo_until_block: int = 0
    ...
    def __post_init__(self) -> None:
        ...
        if self.omit_combo is not None:
            task, stimulus = self.omit_combo
            if task not in (Task.COLOR, Task.SHAPE):
                raise ValueError(f"omit_combo has unknown task {task!r}")
            if not isinstance(stimulus, Stimulus):
                raise ValueError("omit_combo must be (Task, Stimulus)")
            if not 0 <= self.omit_combo_until_block <= self.number_of_blocks:
                raise ValueError(
                    "omit_combo_until_block must lie within [0, number_of_blocks]"
                )
...
def _stimuli_for_block(
    task: Task,
    protocol: EpochProtocol,
    random_generator: np.random.RandomState,
    omit_combo: tuple[Task, Stimulus] | None,
    apply_omission: bool,
) -> list[Stimulus]:
    """Balanced, shuffled stimulus list for one block, dropping the omitted
    combo's stimulus while apply_omission is True and it matches this
    block's task."""

    available = list(ALL_STIMULI)
    if apply_omission and omit_combo is not None and omit_combo[0] == task:
        available = [s for s in available if s != omit_combo[1]]

    indices = np.tile(np.arange(len(available)), protocol.repeats_per_stimulus)
...
def run_blocked_experiment(config: BlockedExperimentConfig) -> ExperimentResult:
    ...
    for block_index in range(config.number_of_blocks):
        ...
        apply_omission = (
            config.omit_combo is not None
            and block_index < config.omit_combo_until_block
        )
        stimuli_for_block = _stimuli_for_block(
            task, config.protocol, random_generator,
            config.omit_combo, apply_omission,
        )
        ...
        # Four stimuli (three while the omitted combo's stimulus is withheld
        # from this task) repeated repeats_per_stimulus times per block.
        for trial_index, stimulus in enumerate(stimuli_for_block):
            trials.append(
                _run_trial(
                    model=model,
                    config=config,
