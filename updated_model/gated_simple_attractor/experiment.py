"""Timing, inputs, results, and the no-cue task-switching experiment.
...
It reshapes published_model/plastic_attractor/experiment.py for the switching
...
class TransitionType(IntEnum):
    ...
    Only two non-first values, unlike the cued siblings: with no cue there is
    no cue-level distinction to make, so every non-first trial is either a
    rule repeat or a rule switch.
    """

    FIRST_TRIAL = -1   # first trial of a block, no predecessor
    RULE_REPEAT = 0    # same rule as before
    RULE_SWITCH = 1    # different rule
...
class EpochProtocol:
    ...
    response window, the relevant feature presented alone in the stimulus
    window, and the response window left at zero input so the network settles
    on a choice. On practice trials the teaching_window drives the response
    units.
    """

    number_of_steps: int = 400
    # kept at the same 50-step window as the sibling packages, purely for
    # comparability -- timing itself isn't the mechanism under test here,
    # only what gets shown during it.
...
# One planned trial: which rule is active and which stimulus was shown. The
# plan is drawn up before the network runs so transition types can be read
# straight off consecutive entries.
@dataclass(frozen=True)
class PlannedTrial:
    task: Task
    stimulus: Stimulus
...
def _balanced_shuffled_deck(
    ...
    """Every stimulus, tiled permutation_repeats times, then shuffled."""

    combos = list(ALL_STIMULI) * permutation_repeats
...
def practice_block_plan(
    ...
    """Plan one practice block: every stimulus, tiled + shuffled.

    Runs through the full permutation permutation_repeats times, so every
    stimulus is taught an equal number of times.
    """

    if permutation_repeats <= 0:
        raise ValueError('practice_permutation_repeats must be positive')

    deck = _balanced_shuffled_deck(permutation_repeats, random_generator)
    return [PlannedTrial(task, stimulus) for stimulus in deck]
...
def switching_practice_block_plan(
    ...
    Unlike real_block_plan's i.i.d. rule draws, this keeps practice's balance
    guarantee: one shuffled stimulus deck per rule, each covering the full
    permutation permutation_repeats times, so every stimulus is still taught
    ...
    decks = {
        task: _balanced_shuffled_deck(permutation_repeats, random_generator)
    ...
    for _ in range(total_trials):
        ...
        stimulus = decks[current_task].pop(0)
        plan.append(PlannedTrial(current_task, stimulus))
...
def real_block_plan(
    ...
    """Plan one real block: rule switches w.p. switch_probability.
    ...
    rule with the given probability. Stimulus identity is independent of the
    rule.
    """
    ...
    return [
        PlannedTrial(task, ALL_STIMULI[int(index)])
        for task, index in zip(tasks, stimulus_indices)
    ]
...
def _transition_type(plan: list[PlannedTrial], index: int) -> TransitionType:
    ...
    return TransitionType.RULE_REPEAT


@dataclass(frozen=True)
class SwitchingExperimentConfig:
    """Every choice needed to reproduce one switching simulation."""

    seed: int = 0
    num_practice_blocks: int = 2
    # matches the original implementation: one instruction trial per
    # stimulus, no repeats.
    ...
    def practice_trials(self) -> int:
        ...
        per_rule = self.practice_permutation_repeats * len(ALL_STIMULI)
...
class ExperimentResult:
    """Trial-level data and final summaries from one switching experiment."""
...
def run_switching_experiment(
    ...
    """Run instruction, then performance practice, then the real blocks."""
    ...
    # real blocks: no teaching, the rule varies trial by trial. learning stays
    # on so the weights keep evolving.
    performance_practice_blocks = (
...
def run_baseline(
    ...
    """Run the standard switching design with plain arguments.
