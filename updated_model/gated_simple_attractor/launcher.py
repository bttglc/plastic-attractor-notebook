"""Run the oracle-suppression switching experiment across seeds and plot results.

OOP counterpart of the flat simulation_launcher.py (which stays in place). It
runs the experiment for each seed, reduces the trials with
gated_simple_attractor's analysis functions, averages across seeds, and
writes the same figures.

Run it from the updated_model folder:

    python gated_simple_attractor/launcher.py
"""
...
# make the gated_simple_attractor package (this file's parent dir) importable
...
# trials per real block (multiple of 4); practice runs the full stimulus
# permutation practice_permutation_repeats times per block
num_trials = 48
...
active_versions = ['baseline', 'cap_inverted']
...
def analyze_and_plot(version_name, results):
    ...
    # since a switching block mixes both rules); xticks stride by 4 (one full
    # ALL_STIMULI permutation, no cue multiplier)
    ...
    # per block kind -- is the withheld signal itself stable across
    # switch-probability conditions, and does it really sit near baseline?
    fig, ax = plt.subplots()
    ...
    # suppression diagnostics per block index, same instruction/real boundaries:
    # does the network fail to respond at all (vs. respond incorrectly), and
    # how do the colour/shape W rows evolve. fixed order
    # colour_shape_row_norms_by_block guarantees: green, blue, square, circle
    row_labels = ['Green', 'Blue', 'Square', 'Circle']
    fig, ax = plt.subplots(1, 2, figsize=(9, 4))
    ax[0].errorbar(block_index, mean_no_response_by_block, yerr=ster_no_response_by_block, color='black', capsize=3, linestyle='-', marker='o')
    ax[0].axvline(num_practice_blocks - .5, color='grey', linestyle='--')
    ax[0].axvline(real_blocks_start - .5, color='grey', linestyle='--')
    ax[0].set_xlabel('Block')
    ax[0].set_ylabel('No-response rate')
    ax[0].set_xticks(block_index)
    ...
    ax[1].axvline(real_blocks_start - .5, color='grey', linestyle='--')
    ax[1].set_xlabel('Block')
    ax[1].set_ylabel('W row norm (colour/shape)')
    ax[1].set_xticks(block_index)
    ax[1].legend()
    fig.tight_layout()
    ...
    # attractor path diagram: stimulus -> conjunction unit -> action, following
    # each conjunction unit's most strongly bound stimulus and action (its
    # learned rule mapping). no cue column, unlike the sibling packages: the
    # only input this model ever presents is the relevant stimulus feature.
    # node positions are normalized fractions per column so the layout stays
    # sensible regardless of number_of_conjunction_units
    ...
    input_y = frac_positions(stimulus_features)
    ...
    ax.set_xlim(-0.6, 2.6)
    ax.set_xticks([0, 1, 2])
    ax.set_xticklabels(['Stimulus', 'Conjunction unit', 'Action'])
    ax.set_yticks([])
