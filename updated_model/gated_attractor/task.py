...
class Vocabulary:
    ...
    @property
    def all_cue_indices(self) -> tuple[int, ...]:
        """Every cue unit index, colour cues then shape cues (input to gating)."""

        return tuple(
            index for task in Task for index in self.cues_by_task[task]
        )

    @property
    def suppressible_feature_indices(self) -> tuple[int, ...]:
        """Colour and shape units (0-3): the only dimension gating may inhibit."""
